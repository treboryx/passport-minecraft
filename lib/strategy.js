...
const getXboxProfile = async (token) => {
  const data = {
  ...
  let profile;
  await fetch("https://xsts.auth.xboxlive.com/xsts/authorize", xstsOptions)
    .then((res) => res.json())
    .then((res) => {
      if (!res.DisplayClaims || res.DisplayClaims === undefined) {
        return done(
      ...
      profile = res.DisplayClaims.xui[0];
    })
    .catch((err) => {
      if (err) {
        return done(
...
const Authorize = async (authenticateToken, done) => {
  ...
  const xboxProfile = await getXboxProfile(authenticateToken);
  ...
  if (!checkOwnership.items.length)
    return done(null, {
      xuid: xboxProfile.xid,
      gamertag: xboxProfile.gtg,
      premium: false,
  ...
  mcProfile.xuid = xboxProfile.xid;
  mcProfile.gamertag = xboxProfile.gtg;
